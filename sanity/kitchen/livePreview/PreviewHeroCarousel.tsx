...
import styles from '../../../components/blocks/HeroCarousel.module.css'
import { PreviewImg } from './PreviewImg'
import { blockDesignStyle } from '../../../lib/content/blockDesignStyle'
import type { HeroCarouselBlock } from '../../../lib/content/types'

/** Live-preview fork of `components/blocks/HeroCarousel.tsx` — same CSS, `next/image` swapped for a plain `<img>`. */
export function PreviewHeroCarousel({ block }: { block: HeroCarouselBlock }) {
      ...
      <div className={styles.frame}>
        {slides.map((slide, i) => (
          <div key={slide.image.url} className={styles.slide} style={{ opacity: i === active ? 1 : 0 }}>
            <PreviewImg src={slide.image.url} alt={slide.image.alt} fill className={styles.image} />
          </div>
        ))}
